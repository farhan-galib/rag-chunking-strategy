...
def render_visual_chunk_map(text: str, chunks: List[Chunk], limit_chunks: int = 20) -> str:
    ...
    if not chunks:
        return "<div style='color: #64748b; font-style: italic;'>No chunks generated.</div>"
    ...
    for i, chunk in enumerate(display_chunks):
        ...
        # The mark itself
        html += f'<mark style="background-color: {color}; border-radius: 3px; padding: 2px 0; border: 1px solid rgba(0,0,0,0.05); cursor: help;" title="Chunk {chunk.chunk_id} ({end-start} chars)">{viewport_text[start:end]}</mark>'
        last_idx = end
...
@st.dialog("🎮 Strategy Simulation", width="large")
def strategy_playground_dialog(analyses: List[Dict]):
    ...
    strategy_guides = {
        "Semantic (AI)": {
            ...
            "params": {
                "max_chunk_size": "Defines the physical limit for each chunk to fit model context windows.",
                "min_chunk_size": "Prevents fragmented chunks (like 'Cont.' or 'Note:') from losing their context."
            }
        },
        ...
        "Sliding Window (Overlap)": {
            ...
            "params": {
                "window_size": "The total physical length of each chunk.",
        ...
        "Fixed Size (Predictable)": {
            ...
            "params": {
                "chunk_size": "The exact character count for each chunk.",
    ...
    with c2:
        st.markdown("#### 🗺️ Chunk Map")
        st.markdown("This map shows the first **15 chunks** generated by the selected strategy.")
        ...
        if 'sim_chunks' in locals():
            ...
            tab1, tab2 = st.tabs(["Raw Text Preview", "Chunk Metadata"])
...
def main():
    ...
    st.markdown("""
        <div style="margin-bottom: 14px;">
            <div style="font-size: 26px; font-weight: 800; color: #0f172a; margin-bottom: 4px;">📄 RAG Chunking Analyzer</div>
            <div style="font-size: 14px; color: #475569; max-width: 760px; line-height: 1.6;">
                Smart document chunking guidance for retrieval-ready AI systems, with quick analysis and clear recommendations.
            </div>
        </div>
    """, unsafe_allow_html=True)

    st.markdown(f"""
        <div class="hero-card" style="padding: 20px 22px; border-bottom: 3px solid #6366f1;">
            <div class="strategy-pill" style="margin-bottom: 10px;">Enterprise AI Optimization</div>
            <div class="strategy-title" style="font-size: 26px; margin-bottom: 10px; line-height: 1.15;">Document IQ & Chunking Strategy Advisor</div>
            <p style="color: #475569; font-size: 14px; margin: 0; max-width: 680px; line-height: 1.65;">
                Analyze documents efficiently and generate compact chunking recommendations that keep your RAG workflows lean and reliable.
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("""
        <div style="font-family: 'Outfit', sans-serif; font-size: 22px; font-weight: 700; color: #0f172a; margin-bottom: 10px;">
            📤 Upload Documents
        </div>
    """, unsafe_allow_html=True)
    ...
    if page == "Upload & Analyze":
        ...
        if file_entries:
            st.markdown("""
                <div style="font-family: 'Outfit', sans-serif; font-size: 20px; font-weight: 700; color: #0f172a; margin-bottom: 12px;">
                    📋 Ready for Analysis
                </div>
            """, unsafe_allow_html=True)
            ...
            with c4:
                if st.button("🎮 Simulation", use_container_width=True, disabled=not has_results):
